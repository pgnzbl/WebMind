...
- **富文本复制**：一键复制包含 base64 图片的 HTML，可直接粘贴到任何网页平台
...
3. **导出和复制文档**
   - 生成完成后，可选择以下方式：
   - **📋 复制文本**：复制纯 Markdown 文本到剪贴板
   - **🎨 复制为富文本**：复制包含图片的 HTML 格式，可直接粘贴到其他网页
     - 自动下载所有图片并转换为 base64
     - 支持粘贴到微信公众号、知乎、掘金等平台
     - 图片会随文本一起粘贴，无需单独上传
   - **💾 下载 Markdown**：保存为 .md 文件
   - **📄 下载 Word**：保存为 .docx 文件，图片完整嵌入
...
### Q: 复制为富文本后粘贴到网页，为什么图片显示不正常？
**A**: 
1. **目标平台不支持 base64 图片**：少数平台可能会过滤 base64 图片，建议使用该平台的图片上传功能
2. **图片过大**：单张图片 > 5MB 可能导致粘贴缓慢或失败，建议压缩图片
3. **剪贴板限制**：某些浏览器或系统对剪贴板大小有限制，建议单次复制的总内容 < 10MB
4. **格式兼容性**：如遇问题，可尝试先粘贴到本地编辑器（如 VS Code）预览效果

## 🔄 更新日志

### v2.1.0 (2025-01-10)
- 🎨 **新增复制为富文本功能**
  - 一键复制包含 base64 图片的 HTML 格式
  - 完美支持粘贴到微信公众号、知乎、掘金等网页平台
  - 自动下载所有图片并转换为 base64 嵌入
  - 实时进度显示（图片下载进度）
  - 图片会随文本一起粘贴，无需单独上传
  - 解决从 Word 复制粘贴时图片路径变为 `file:///` 的问题
- ✨ **样式优化**
  - 富文本默认左对齐，适配各类网页编辑器
  - 添加悬停提示，更好的用户体验
  - 支持禁用状态的视觉反馈
- 🔧 **技术改进**
  - 使用 Clipboard API 支持多格式复制（HTML + 纯文本）
  - 错误处理和超时保护
  - 优化图片下载逻辑
