...
### v2.0.3 (2025-01-09)
- 🚀 **Markdown 渲染器重大升级**
  - 集成成熟稳定的 marked.js v11.1.1 库
  - 彻底解决占位符被破坏导致的显示问题
  - 修复行内代码（如 `pyeval`）后文字倾斜问题
  - 修复代码块显示为 CODEBLOCK0、CODEBLOCK1 占位符的问题
  - 完美支持所有标准 Markdown 语法
  - 代码从 150+ 行优化到 125 行，维护成本大幅降低
- ✨ **渲染质量提升**
  - 代码块正确换行，每行独立显示
  - 行内代码与普通文本完美配合，不互相干扰
  - 混合格式（粗体+斜体+代码）正确渲染
  - 支持语言标识（```bash、```javascript 等）

### v2.0.2 (2025-01-09)
- 📐 **Word 格式优化**
  - 标题字体：统一使用黑体（SimHei），字号分级明确（H1: 16pt, H2: 14pt, H3-H6: 12pt）
  - 正文字体：统一使用宋体（SimSun），小四号（12pt）
  - 代码字体：Consolas 等宽字体，灰色背景
  - 去除默认段落缩进，确保文档格式专业规范
  - 符合 GB/T 1.1-2020 中文文档格式标准
- 🔧 **Markdown 渲染器增强**
  - 新增有序列表支持（1. 2. 3. 格式）
  - 修复列表项不换行问题，确保每项独占一行
  - 优化列表渲染逻辑，正确区分有序和无序列表
